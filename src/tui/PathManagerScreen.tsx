...
export function PathManagerScreen({
  ...
  const addPath = useCallback(
    (rawPath: string) => {
      ...
      try {
        ...
        const exists = targets.some((t) => t.path.toLowerCase() === scope.targetPath.toLowerCase());
        if (exists) {
  ...
  const removeSelected = useCallback(() => {
    if (targets.length === 0 || selectedIndex >= targets.length) {
      return;
    }
  ...
  return (
    <Box flexDirection="column" paddingX={2}>
        ...
        {targets.length === 0 ? (
          ...
          <Box flexDirection="column" marginTop={1}>
            {targets.map((t, i) => {
              ...
              const displayPath = t.path.length > 60 ? "..." + t.path.slice(-57) : t.path;
              return (
                <Text key={`${t.path}-${i}`}>
                  <Text color={isSelected ? "#D4A017" : "transparent"}>
                    {isSelected ? "▸ " : "  "}
                  </Text>
                  <Text color={t.enabled ? "#52B788" : "#6C757D"}>{t.enabled ? "✓" : "○"}</Text>
                  <Text
                    color={isSelected ? "#E2DCC8" : t.enabled ? "#8D99AE" : "#6C757D"}
                    bold={isSelected}
                  >
                    {" "}
      ...
      <Box justifyContent="space-between" width={68}>
        <Text color="#6C757D">
          {targets.length} target(s), {enabledCount} enabled
        </Text>
        <Text color="#8D99AE">A Add · D Remove · Space Toggle · Enter Start · Esc Back</Text>
      </Box>
    </Box>
  );
}
